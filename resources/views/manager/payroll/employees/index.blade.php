...
@section('content')
    <div
        ...
        <div>
            <h2 style="font-size: 1.25rem; font-weight: 700; color: var(--white); margin-bottom: 0.25rem;">Employee Master
                Database</h2>
            <p style="font-size: 0.82rem; color: var(--text-muted); margin: 0;">Manage clinic staff compensation rates,
                positions, and statutory government identities.</p>
        </div>
    ...
    <div class="card" style="margin-bottom: 1.5rem;">
        <div class="card-body" style="padding: 1rem 1.25rem;">
            <form method="GET" action="{{ route('manager.payroll.employees.index') }}"
                style="display: flex; gap: 1rem; flex-wrap: wrap; align-items: center;">
                <div style="flex: 1; min-width: 200px;">
                    <input type="text" name="search" class="form-control"
                        placeholder="Search by name, employee # or email..." value="{{ request('search') }}">
                </div>
                    ...
                    <tbody>
                        @forelse($employees as $emp)
                            <tr>
                                <td>
                                    <span
                                        style="font-family: monospace; font-weight: 700; color: var(--gold-light); font-size: 0.85rem;">
                                        {{ $emp->employee_code }}
                                    </span>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div style="font-size: 0.72rem; color: var(--text-muted);">
                                        ...
                                        PhilHealth: <span
                                            style="color: var(--white);">{{ $emp->philhealth_no ?: '-' }}</span><br>
                                ...
                                <td>
                                    <div style="display: flex; gap: 0.35rem;">
                                        ...
                                        <form action="{{ route('manager.payroll.employees.destroy', $emp) }}" method="POST"
                                            onsubmit="return confirm('Are you sure you want to remove this employee?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-ghost btn-sm"
                                                style="padding: 0.25rem 0.5rem; color: #ef4444;" title="Delete">
                                                🗑️
                                            </button>
                                        </form>
                                    </div>
                                ...
                                <div class="modal-dialog modal-lg" style="max-width: 920px; width: 95%;">
                                    <div class="modal-header">
                                        <div class="modal-title-group">
                                            ...
                                            <span style="font-size: 0.75rem; color: var(--gold-light);">Code:
                                                {{ $emp->employee_code }}</span>
                                        </div>
                                        <button type="button" class="modal-close-btn" data-modal-close>&times;</button>
                                    </div>
                                    <form action="{{ route('manager.payroll.employees.update', $emp) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                                ...
                                                <div
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <input type="number" step="0.01" name="basic_salary"
                                                            class="form-control" value="{{ $emp->basic_salary }}"
                                                            required>
                                                    </div>
                                                    <div class="form-group">
                                                ...
                                                <div
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <select name="status" class="form-control" required>
                                                            <option value="active"
                                                                {{ $emp->status == 'active' ? 'selected' : '' }}>Active
                                                            </option>
                                                            <option value="inactive"
                                                                {{ $emp->status == 'inactive' ? 'selected' : '' }}>Inactive
                                                            </option>
                                        ...
                                        <div class="modal-footer" style="padding: 1.25rem 1.75rem;">
                                            ...
                                            <button type="submit" class="btn btn-gold"
                                                style="padding: 0.6rem 1.5rem;">Update Employee Details</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
        ...
        <div class="modal-dialog modal-lg" style="max-width: 920px; width: 95%;">
            <div class="modal-header">
                <div class="modal-title-group">
                    ...
                    <span style="font-size: 0.75rem; color: var(--gold-light);">Automatic Employee ID generation
                        (EMP-YYYY-XXXX)</span>
                </div>
                <button type="button" class="modal-close-btn" data-modal-close>&times;</button>
            </div>
            <form action="{{ route('manager.payroll.employees.store') }}" method="POST">
                @csrf
                <div class="modal-body" style="padding: 1.75rem; max-height: 80vh; overflow-y: auto;">
                    ...
                    <div style="margin-bottom: 1.25rem;">
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                            <div class="form-group">
                                <label class="form-label">First Name *</label>
                                <input type="text" name="first_name" class="form-control" placeholder="e.g. Arnel"
                                    required>
                            </div>
                            <div class="form-group">
                                <label class="form-label">Last Name *</label>
                                <input type="text" name="last_name" class="form-control" placeholder="e.g. Bautista"
                                    required>
                            </div>
                        </div>
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-top: 0.75rem;">
                            <div class="form-group">
                                <label class="form-label">Email Address</label>
                                <input type="email" name="email" class="form-control"
                                    placeholder="employee@sanmodesto.com">
                            </div>
                            <div class="form-group">
                                <label class="form-label">Contact Phone</label>
                ...
                <div class="modal-footer" style="padding: 1.25rem 1.75rem;">
                    <button type="button" class="btn btn-ghost" data-modal-close>Cancel</button>
                    <button type="submit" class="btn btn-gold" style="padding: 0.6rem 1.5rem;">Save Employee
                        Record</button>
                </div>
            </form>
        </div>
