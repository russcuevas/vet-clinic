...
@section('content')
        ...
        <div style="display: flex; gap: 0.75rem; align-items: center; flex-wrap: wrap;">
            ...
            <div style="display: flex; align-items: center; background: rgba(255, 255, 255, 0.05); border: 1px solid var(--navy-border); border-radius: 6px; padding: 0.2rem 0.5rem; gap: 0.35rem;">
                ...
                @foreach([$year - 1, $year, $year + 1] as $yr)
                    <a href="{{ route('manager.payroll.leaves.index', array_merge(request()->query(), ['year' => $yr])) }}"
    ...
    <div class="card" style="margin-bottom: 1.5rem;">
        <div class="card-header">
            ...
            <div style="display: flex; gap: 0.5rem; align-items: center; flex-wrap: wrap;">
                <form action="{{ route('manager.payroll.leaves.archive') }}" method="POST" style="display: inline;">
                    @csrf
                ...
                <a href="{{ route('manager.payroll.leaves.print_ledger', $year) }}" target="_blank" class="btn btn-gold btn-sm">
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @elseif($item['sl_unused'] > 0)
                                        <form action="{{ route('manager.payroll.leaves.convert_sl') }}" method="POST" style="display: inline;"
    ...
    <div class="card">
        <div class="card-header">
            ...
            <div style="display: flex; gap: 0.5rem;">
                <a href="{{ route('manager.payroll.leaves.index') }}" class="btn btn-ghost btn-sm {{ !request('status') ? 'btn-gold' : '' }}">All</a>
                <a href="{{ route('manager.payroll.leaves.index', ['status' => 'pending']) }}" class="btn btn-ghost btn-sm {{ request('status') == 'pending' ? 'btn-gold' : '' }}">Pending</a>
                <a href="{{ route('manager.payroll.leaves.index', ['status' => 'approved']) }}" class="btn btn-ghost btn-sm {{ request('status') == 'approved' ? 'btn-gold' : '' }}">Approved</a>
            </div>
        </div>
        <div class="card-body" style="padding: 0;">
            <div class="table-responsive">
                        ...
                        @forelse($leaves as $leave)
                            <tr>
                                ...
                                <td>
                                    <div style="display: flex; gap: 0.35rem;">
                                        ...
                                        <form action="{{ route('manager.payroll.leaves.destroy', $leave) }}" method="POST"
                            ...
                            <div class="modal-backdrop" id="modal-status-leave-{{ $leave->id }}">
                                <div class="modal-dialog" style="max-width: 480px;">
                                    ...
                                    <form action="{{ route('manager.payroll.leaves.status', $leave) }}" method="POST">
                                        @csrf
                                        @method('PUT')
    ...
    <div class="modal-backdrop" id="modal-file-leave">
        <div class="modal-dialog modal-lg" style="max-width: 860px;">
            ...
            <form action="{{ route('manager.payroll.leaves.store') }}" method="POST">
                @csrf
