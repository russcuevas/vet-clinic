...
@section('content')
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem; flex-wrap: wrap; gap: 1rem;">
        ...
        <form method="GET" action="{{ route('manager.payroll.annual') }}" style="display: flex; gap: 0.5rem; align-items: center;">
            <select name="year" class="form-control" onchange="this.form.submit()">
                @for($y = now()->year - 3; $y <= now()->year + 1; $y++)
                    <option value="{{ $y }}" {{ $year == $y ? 'selected' : '' }}>Year {{ $y }}</option>
                @endfor
            </select>
            <a href="{{ route('manager.payroll.periods.index') }}" class="btn btn-navy btn-sm">Cycles</a>
        </form>
    </div>
