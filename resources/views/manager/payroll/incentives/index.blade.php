...
@section('content')
            ...
            @forelse($rules as $rule)
                ...
                <div class="modal-backdrop" id="modal-edit-rule-{{ $rule->id }}">
                    <div class="modal-dialog" style="max-width: 520px;">
                        ...
                        <form action="{{ route('manager.payroll.incentives.rules.update', $rule) }}" method="POST">
                            @csrf
                            @method('PUT')
    ...
    <div class="card" style="margin-bottom: 1.5rem;">
        <div class="card-body" style="padding: 1rem 1.25rem; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem;">
            <form method="GET" action="{{ route('manager.payroll.incentives.index') }}" style="display: flex; gap: 0.75rem; flex-wrap: wrap; align-items: center;">
                <div>
                    ...
                    <tbody>
                        @foreach($incentives as $inc)
                            <tr>
                                ...
                                <td>
                                    <form action="{{ route('manager.payroll.incentives.destroy', $inc) }}" method="POST"
    ...
    <div class="modal-backdrop" id="modal-add-employee-incentive">
        <div class="modal-dialog" style="max-width: 600px;">
            ...
            <form action="{{ route('manager.payroll.incentives.records.store') }}" method="POST" id="incentiveForm">
                @csrf
                <div class="modal-body" style="padding: 1.5rem;">
                    <div class="form-group">
    ...
    <div class="modal-backdrop" id="modal-add-rule">
        <div class="modal-dialog" style="max-width: 520px;">
            ...
            <form action="{{ route('manager.payroll.incentives.rules.store') }}" method="POST">
                @csrf
                <div class="modal-body" style="padding: 1.5rem;">
    ...
    <script>
        ...
        async function fetchClinicStats() {
            ...
            try {
                const response = await fetch(`{{ route('manager.payroll.incentives.suggest') }}?employee_id=${empId}&month=${month}&year=${year}`);
