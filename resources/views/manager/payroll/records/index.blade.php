...
@section('content')
    ...
    <div class="card" style="margin-bottom: 1.5rem;">
        <div class="card-body" style="padding: 1rem 1.25rem; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem;">
            <div style="display: flex; gap: 0.5rem;">
                <a href="{{ route('manager.payroll.periods.index') }}" class="btn btn-ghost btn-sm {{ !request('type') ? 'btn-gold' : '' }}">
                    All Cycles
                </a>
                <a href="{{ route('manager.payroll.periods.index', ['type' => '15_days']) }}" class="btn btn-ghost btn-sm {{ request('type') === '15_days' ? 'btn-gold' : '' }}">
                    🗓️ 15 Days (Semi-Monthly)
                </a>
                <a href="{{ route('manager.payroll.periods.index', ['type' => '30_days']) }}" class="btn btn-ghost btn-sm {{ request('type') === '30_days' ? 'btn-gold' : '' }}">
                    📅 30 Days (Monthly)
                </a>
            </div>
            <a href="{{ route('manager.payroll.annual') }}" class="btn btn-navy btn-sm">
                📊 View Annual Records
            </a>
        </div>
    </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div style="display: flex; gap: 0.35rem;">
                                        <a href="{{ route('manager.payroll.periods.show', $period) }}" class="btn btn-navy btn-sm" style="padding: 0.3rem 0.6rem;">
                                            Review & Payslips
                                        </a>
                                        <a href="{{ route('manager.payroll.summary.print', $period) }}" target="_blank" class="btn btn-ghost btn-sm" style="padding: 0.3rem 0.5rem;" title="Print Summary Report">
                                            🖨️
                                        </a>
                                        <form action="{{ route('manager.payroll.periods.destroy', $period) }}" method="POST"
    ...
    <div class="modal-backdrop" id="modal-generate-payroll">
        <div class="modal-dialog" style="max-width: 550px;">
            ...
            <form action="{{ route('manager.payroll.periods.generate') }}" method="POST">
                @csrf
