...
@php
    ...
    $headerTitle = 'Pet Grooming Desk';
    $breadcrumb = 'Cashier / Grooming Services';
@endphp
...
@section('content')
    ...
    <div class="table-toolbar">
        <div style="display: flex; gap: 0.75rem; align-items: center; flex-wrap: wrap;">
            ...
            <div style="display: flex; gap: 0.35rem;">
                <a href="{{ route('cashier.grooming.index') }}" class="btn btn-sm {{ !request('status') ? 'btn-gold' : 'btn-navy' }}">All</a>
                <a href="{{ route('cashier.grooming.index', ['status' => 'queued']) }}" class="btn btn-sm {{ request('status') === 'queued' ? 'btn-gold' : 'btn-navy' }}">Queued</a>
                <a href="{{ route('cashier.grooming.index', ['status' => 'in_progress']) }}" class="btn btn-sm {{ request('status') === 'in_progress' ? 'btn-gold' : 'btn-navy' }}">In Progress</a>
                <a href="{{ route('cashier.grooming.index', ['status' => 'completed']) }}" class="btn btn-sm {{ request('status') === 'completed' ? 'btn-gold' : 'btn-navy' }}">Completed</a>
            </div>
        </div>
    ...
    <div class="card">
        <div class="card-header">
            <div class="card-title-group">
                ...
                <span class="card-subtitle">Process grooming requests, styling details, notes, and auto-queue to checkout</span>
            </div>
        </div>
                    ...
                    <tbody>
                        @forelse($records as $groom)
                            <tr>
                                ...
                                <td>
                                    <strong style="color: var(--gold-primary);">₱{{ number_format($groom->price, 2) }}</strong>
                                </td>
                                <td>
                                ...
                                <td style="text-align: right;">
                                    <div style="display: inline-flex; gap: 0.4rem;">
                                        ...
                                        <button type="button" class="btn btn-danger btn-sm"
                                            ...
                                            data-action-url="{{ route('cashier.grooming.destroy', $groom->id) }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="9" style="text-align: center; padding: 3rem 1rem; color: var(--text-muted);">
                                    ...
                                    <div style="font-weight: 600; color: var(--white);">No grooming records found</div>
                                    <p style="font-size: 0.82rem; margin-top: 0.25rem;">Click 'New Grooming Session' to record a grooming request.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <div class="modal-backdrop" id="modal-new-grooming">
        <div class="modal-dialog modal-lg">
            ...
            <form action="{{ route('cashier.grooming.store') }}" method="POST">
                @csrf
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <span style="font-size: 0.72rem; color: var(--text-muted);">Will be automatically queued into Checkout & Billing.</span>
                    </div>
                </div>
            ...
            <form method="POST" action="">
                ...
                <div class="modal-body">
                    <p style="color: var(--text-secondary); font-size: 0.9rem;">
                        Are you sure you want to delete grooming record <strong style="color: var(--white);" data-bind="target_name"></strong>?
                    </p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-ghost" data-modal-close>Cancel</button>
