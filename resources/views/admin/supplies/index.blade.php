        ...
        <div class="card">
            <div class="card-header">
                <div class="card-title-group">
                    <h3 class="card-title">Pet Supplies Catalog</h3>
                    ...
                    @foreach($supplies as $item)
                        <div class="supply-item-card" 
                            ...
                            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 0.5rem;">
                                ...
                                <span style="font-size: 0.75rem; color: {{ $item->stock_quantity <= 5 ? 'var(--danger)' : 'var(--text-muted)' }};">
                                    Stock: {{ $item->stock_quantity }} {{ $item->unit }}
                                </span>
                            </div>
                            <h4 style="font-size: 0.9rem; font-weight: 700; color: var(--white); margin-bottom: 0.35rem;">{{ $item->name }}</h4>
                            <p style="font-size: 0.75rem; color: var(--text-muted); margin-bottom: 0.75rem;">{{ Str::limit($item->description, 45) }}</p>
                            <div style="display: flex; align-items: center; justify-content: space-between;">
                                <strong style="color: var(--gold-primary); font-size: 1.05rem;">₱{{ number_format($item->unit_price, 2) }}</strong>
                                <button type="button" class="btn btn-gold btn-sm" style="padding: 3px 8px;">+ Add</button>
                            </div>
                        </div>
                    @endforeach
                </div>
